...
const AdminDashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // user_roles tablosundan kullanıcı rolünü kontrol et
    const checkUserRole = async () => {
      ...
      if (error || !data || data.role !== "admin") {
        router.push("/"); // admin değilse anasayfaya yönlendir
        return;
      }

      fetchCounts(); // sadece adminse sayfa yükleme verilerini çek
    };
  ...
  const fetchCounts = async () => {
    ...
    if (!pendingError) setPendingApprovalCount(pendingCount || 0);
